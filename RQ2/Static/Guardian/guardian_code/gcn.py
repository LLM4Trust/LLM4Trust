...
class GCNTrainer(object):
    ...
    def setup_dataset(self):
        ...
        if self.args.stratified_split:
            stratSplit = StratifiedShuffleSplit(n_splits=1, test_size=self.args.test_size, random_state=self.args.seed)
        ...
        print("Number of test edges: ", len(self.test_edges))
        ...
        self.train_edges = torch.from_numpy(np.array(self.train_edges, dtype=np.int64).T).type(torch.long).to(self.device)  # [2, 43505]
        self.y_train = torch.from_numpy(np.array(self.y_train, dtype=np.float32)).type(torch.float).to(self.device)  # [43505, 4]
    ...
    def create_and_train_model(self):
        ...
        self.model = GraphConvolutionalNetwork(self.device, self.args, self.X, self.num_labels, mask).to(self.device)
